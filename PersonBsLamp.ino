...
void connectToBroker() {
  while (!client.connected()) {
    ...
    client.connect("personBsLamp", mqttUsername, mqttPassword);
    if (client.connect("personBsLamp")) {
      pulse(5);
      Serial.println("Connected.");
      client.publish("ConnectionInfo", "Hello world from personBsLamp");
      client.subscribe("ConnectionInfo");
      client.subscribe("personBsLampColor");     //the MQTT topic for this lamp's color
    }
    else {
      delay(1000);
...
void callback(char* topic, byte* payload, unsigned int length) {
  ...
  if (strcmp(topic, "personBsLampColor") == 0) {
  ...
  if (strcmp(topic, "ConnectionInfo") == 0) { //if the topic is ConnectionInfo, remind the other lamp what their color should be
    client.publish("personAsLampColor", sendColorChar);
...
void loadcerts() {
  ...
  File cert = SPIFFS.open("/personBslampclient.crt", "r");
  if (cert) {
  ...
  File privateKey = SPIFFS.open("/personBslampclient.key", "r");
  if (privateKey) {
...
void handleButton() {
  ...
  if (button.read() && millis() - pressStart >= delayToSend && willSend) { //the button is not depressed AND it's been delayToSend since the last press AND we're supposed to send a color
    ...
    client.publish("personAsLampColor", sendColorChar);
    Serial.println("Publishing sendColorChar to personAsLampColor.");
